...
[Toolchain Map](../concepts/README.md) has its own diagram of the same
...
- **[Toolchain Map](../concepts/README.md)** -- every piece named above,
  in one table, with what each hands to the next.
- **[Modelling](../modelling/README.md)** -- write your own `Problem`
  instead of reading someone else's; builds on a second running example,
  Knapsack, from the first input declaration onward.
- **[Running Programs](../running/README.md)** -- drive an already-compiled
  program from Python yourself, the way `runner.py` does internally.
- **[Solving Overview](../solving/README.md)** -- the five solvers behind
