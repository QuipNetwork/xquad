...
| `0x69` | `ROWSUM` | `reg` | `[..., row] → [..., sum]` | `read` — sums `reg.xqmx.linear` across row | Pop `row`. Push `Σ linear[row*cols + c]` for `c` in `0..cols`. The sum is accumulated in column order with every partial sum range-checked; overflow raises `ArithmeticOverflow` (SPEC.md overflow rule). |
| `0x6A` | `COLSUM` | `reg` | `[..., col] → [..., sum]` | `read` — sums `reg.xqmx.linear` down column | Pop `col`. Push `Σ linear[r*cols + col]` for `r` in `0..rows`. The sum is accumulated in row order with every partial sum range-checked; overflow raises `ArithmeticOverflow` (SPEC.md overflow rule). |
...
| `0x75` | `ATLEAST` | `model indices` | `[..., k, penalty] → [...]` | `read` indices; `mutate` model (grows size) | Pop `penalty`, then `k`. Read variable indices from `indices` (`vec<int>`). Allocate slack variables at `model.size`. Apply `EQUALITY` expansion with negative slack coefficients and target = k. Error if `k <= 0` or `k > len(indices)`: the `xqvm_py` reference implementation raises `ValueError`; the Rust `xqvm` VM raises `IndexOutOfBounds`; its error type has no `ValueError` variant. See [derivation](HLF.md#atleast-derivation). |
| `0x76` | `ATLEASTW` | `model indices coeffs` | `[..., k, penalty] → [...]` | `read` indices, coeffs; `mutate` model (grows size) | Pop `penalty`, then `k`. Read variable indices from `indices` (`vec<int>`) and weights from `coeffs` (`vec<int>`). Allocate slack variables at `model.size`. Apply `EQUALITY` expansion with weighted variables, negative slack coefficients, and target = k. Lengths of `indices` and `coeffs` must match; error otherwise: `xqvm_py` raises `ValueError`, the Rust `xqvm` VM raises `VecLengthMismatch`. Error if `k <= 0`: `xqvm_py` raises `ValueError`, the Rust `xqvm` VM raises `IndexOutOfBounds`. The weight sum and its excess over `k` are accumulated in index order with every partial value range-checked; overflow raises `ArithmeticOverflow`. See [derivation](HLF.md#atleastw-derivation). |
| `0x77` | `REDUCE` | `model` | `[..., var_a, var_b, P_aux] → [..., aux]` | `mutate` model (grows size) | Pop `P_aux`, then `var_b`, then `var_a`. Allocate auxiliary variable w at `model.size` (size grows by 1). Add enforcement terms constraining `w = x_a × x_b`. Push the index of w. Error if `var_a` or `var_b` out of range `[0, model.size)` before allocation: `xqvm_py` raises `ValueError`, the Rust `xqvm` VM raises `IndexOutOfBounds`. See [derivation](HLF.md#reduce-derivation). |
