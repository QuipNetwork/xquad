...
**Size precondition.** Every allocator that takes a `size` from the stack -- `BQMX`, `SQMX`, `XQMX`, `BSMX`, `SSMX`, `XSMX` -- requires that size to be non-negative, and rejects a negative one with `InvalidAllocation` before anything else happens. A non-negative size is then charged against the [allocation budget](SPEC.md#allocation-budget), raising `MemoryLimitExceeded` if it does not fit, and only after the charge is it required to be no greater than the maximum allocator size `2^32 - 1` -- a size past the maximum raises `InvalidAllocation`. Either fault leaves the target register alone.

The two halves of the precondition sit either side of the charge on purpose, and the order is observable: under any budget below 32 GiB a size past the maximum exhausts the budget first, so the fault is `MemoryLimitExceeded` and not `InvalidAllocation`. The maximum bounds a grown model size too -- see `ATLEAST`, `ATLEASTW`, `REDUCE` and `EQUALITY` below, each of which appends variables to a model and range-checks the result the same way. The order is normative (see [Allocator validation order](SPEC.md#allocation-budget)): validating and charging off the operand as the program pushed it, rather than off a value already narrowed to the executing target's native width, is what makes a five-billion-variable request raise the same fault on a 32-bit target as on a 64-bit one, instead of quietly becoming an empty model that costs nothing. The rows below name only what is theirs.
