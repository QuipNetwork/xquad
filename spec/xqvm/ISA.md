...
| `0x76` | `ATLEASTW` | `model indices coeffs` | `[..., k, penalty] → [...]` | `read` indices, coeffs; `mutate` model (grows size) | Pop `penalty`, then `k`. Read variable indices from `indices` (`vec<int>`) and weights from `coeffs` (`vec<int>`). Allocate slack variables at `model.size`. Apply `EQUALITY` expansion with weighted variables, negative slack coefficients, and target = k. Lengths of `indices` and `coeffs` must match; error otherwise: `xqvm_py` raises `ValueError`, the Rust `xqvm` VM raises `VecLengthMismatch`. Error if `k <= 0`: `xqvm_py` raises `ValueError`, the Rust `xqvm` VM raises `IndexOutOfBounds`. See [derivation](HLF.md#atleastw-derivation). |
