...
- **The counter is a u64 and never saturates.** A charge that would carry it past `2^64 - 1` raises `StepLimitExceeded` whatever the limit, so an unbounded budget is a limit of `2^64 - 1` and the count a run reports is always the count it spent. See [METERING.md](METERING.md#the-step-counter).
