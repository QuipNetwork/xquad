...
`input_slots` and `output_slots` are informational and are not validated by the decoder. Each counts *instructions* of that kind in the stream, saturating at 255. Neither is a slot count. A program with three `OUTPUT` instructions that all write slot 0 records `output_slots = 3` against a required slot count of 1; a program with one `OUTPUT` inside a loop that writes slots 0 through 9 records `output_slots = 1` against a required count of 10. Neither byte can therefore be used to pre-size a calldata or output-slot array. The host fixes both counts before the run, and a slot outside them raises `CallDataIndex` or `OutputIndex` at run time. That saturation is a second and independent bound. Each field is one byte, so no count above 255 is representable in it at all, and a host whose slot ceiling is 256 or more could not express that ceiling here even in a format where these counts did mean slots. Redefining the bytes to carry slot arity is therefore not the small change it looks like. Whether a program should declare its own output arity at all -- in a wider field, or by some mechanism other than a header count -- is left open rather than answered here.
