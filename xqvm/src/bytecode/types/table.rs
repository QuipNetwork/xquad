...
///
/// ```text
/// (code, Variant, "MNEMONIC", "doc", stack_effect, {field_name: FieldType, ...})
/// ```
///
...
/// | `stack_effect` | [`StackEffect`](crate::StackEffect) expr | Values popped, then values pushed: `StackEffect::fixed(pops, pushes)`. `SCLR` alone is `StackEffect::Reset`, which empties the stack whatever its depth. |
...
///     ( $( ($code:literal, $var:ident, $mnem:literal, $doc:literal, $stack:expr, {$($f:tt)*}) ),* $(,)? ) => {
///         &[ $( $mnem ),* ]
///     }
/// }
///
    ...
    ($mac:ident) => {
        $mac! {
            ...
            (0x41, Sqmx,    "SQMX",     "Pop size; allocate a spin Ising model ({-1, +1} domain) into a register.",
             $crate::StackEffect::fixed(1, 0), {reg: $crate::Register}),
            (0x42, Xqmx,    "XQMX",     "Pop k then size; allocate an integer model with domain {0, ..., k-1} into a register. Errors when k < 2.",
             $crate::StackEffect::fixed(2, 0), {reg: $crate::Register}),
            ...
            (0x44, Ssmx,    "SSMX",     "Pop size; allocate a spin sample ({-1, +1} domain) into a register.",
             $crate::StackEffect::fixed(1, 0), {reg: $crate::Register}),
            (0x45, Xsmx,    "XSMX",     "Pop k then size; allocate an integer sample with domain {0, ..., k-1} into a register. Errors when k < 2.",
             $crate::StackEffect::fixed(2, 0), {reg: $crate::Register}),
            // ---------------------------------------------------------------
            // Vec Allocators
            // ---------------------------------------------------------------
            ...
            (0x51, VecGet,  "VECGET",   "Pop index; push `vec[index]` from the register's vec.",
             $crate::StackEffect::fixed(1, 1), {reg: $crate::Register}),
            ...
            (0x54, Slack,   "SLACK",    "Pop capacity and `start_index`; append slack variable indices and power-of-two coefficients to two register vecs.",
             $crate::StackEffect::fixed(2, 0), {indices: $crate::Register, coeffs: $crate::Register}),
            // ---------------------------------------------------------------
            // Index Math
            // ---------------------------------------------------------------
            (0x5A, IdxGrid, "IDXGRID",  "Pop cols, col, row; push the flat grid index row * cols + col.",
             $crate::StackEffect::fixed(3, 1), {}),
            ...
            (0x63, GetQuad, "GETQUAD",  "Pop j and i; push `quadratic[i, j]` from the register's model (0 if absent).",
             $crate::StackEffect::fixed(2, 1), {reg: $crate::Register}),
            (0x64, SetQuad, "SETQUAD",  "Pop value, j, and i; set `quadratic[i, j]` in the register's model.",
             $crate::StackEffect::fixed(3, 0), {reg: $crate::Register}),
            ...
            (0x74, Equality, "EQUALITY", "Pop penalty and target; expand weighted equality constraint into QUBO terms on a model.",
             $crate::StackEffect::fixed(2, 0), {model: $crate::Register, indices: $crate::Register, coeffs: $crate::Register}),
            (0x75, AtLeast,  "ATLEAST",  "Pop penalty and k; allocate slack variables and apply at-least-k constraint.",
             $crate::StackEffect::fixed(2, 0), {model: $crate::Register, indices: $crate::Register}),
            ...
            (0x77, Reduce,   "REDUCE",   "Pop `P_aux`, `var_b`, `var_a`; allocate auxiliary variable and add Rosenberg enforcement terms; push aux index.",
             $crate::StackEffect::fixed(3, 1), {model: $crate::Register}),
            (0x7F, Energy,   "ENERGY",   "Compute the Hamiltonian energy of a sample against a model; push the result.",
             $crate::StackEffect::fixed(0, 1), {model: $crate::Register, sample: $crate::Register}),
            // ---------------------------------------------------------------
            // Special
            //
