...
def mock_gpu_env(monkeypatch):
    """Inject fake dwave.samplers and numba.cuda to avoid GPU hardware in tests."""
    mock_first = MagicMock()
    mock_first.sample = {0: 0, 1: 0}
    mock_first.energy = 0.0
    mock_first.num_occurrences = 1
    ...
    fake_dwave_samplers = types.ModuleType("dwave.samplers")
    fake_dwave_samplers.SimulatedAnnealingSampler = MagicMock(return_value=mock_sampler)

    fake_dwave = types.ModuleType("dwave")
    fake_dwave.samplers = fake_dwave_samplers
    ...
    monkeypatch.setitem(sys.modules, "dwave", fake_dwave)
    monkeypatch.setitem(sys.modules, "dwave.samplers", fake_dwave_samplers)
    monkeypatch.setitem(sys.modules, "numba", fake_numba)
    monkeypatch.setitem(sys.modules, "numba.cuda", fake_cuda)

    return {
        "dwave_samplers": fake_dwave_samplers,
...
class TestSolverGPUSA:
    ...
    def test_solve_binary(self, mock_gpu_env) -> None:
        """Solve a trivial 2-variable QUBO with mocked GPU sampler."""
        model = XQMX.binary_model(2)
    ...
    def test_solve_spin(self, mock_gpu_env) -> None:
        """Solve a 2-variable Ising model with mocked GPU sampler."""
        mock_gpu_env["first"].sample = {0: -1, 1: -1}
        mock_gpu_env["first"].energy = -2.0
        model = XQMX.spin_model(2)
        model.set_linear(0, -1)
        model.set_linear(1, -1)
    ...
    def test_solve_kwargs_override(self, mock_gpu_env) -> None:
        """Per-call kwargs override constructor defaults."""
        model = XQMX.binary_model(2)
    ...
    def test_no_gpu_raises(self, mock_gpu_env) -> None:
